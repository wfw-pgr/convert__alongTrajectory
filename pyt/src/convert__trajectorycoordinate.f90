...
! ====================================================== !
subroutine convert__trajectorycoordinate( trajec, coord_xy, coord_sn, ntrajec, ncoord )
  implicit none
  integer         , intent(in)  :: ntrajec, ncoord
  double precision, intent(in)  :: coord_xy(3,ncoord), trajec(3,ntrajec)
  double precision, intent(out) :: coord_sn(3,ncoord)
  integer                       :: icoord
  double precision              :: svalue, nvalue, xtarget(3), xcross(3)
  integer         , parameter   :: s_=1, n_=2

  xcross(:)  = 0.d0
  xtarget(:) = 0.d0
  do icoord=1, ncoord
     xtarget(1:3)    = coord_xy(1:3,icoord)
     call find__perpendicularLine( xtarget, trajec, svalue, nvalue, xcross, ntrajec )
     coord_sn(s_,icoord) = svalue
     coord_sn(n_,icoord) = nvalue
  enddo
...
contains
  ...
  subroutine find__perpendicularLine( xtgt, trajectory, sval, nval, xcrs, ntrajectory )
    ...
    double precision              :: Amatrix(4,4), xpos(4), ypos(4), xcoef(4), ycoef(4)
    ...
    do iter=1, iterMax
       ...
       if ( ic.ne.ic_old ) then
          do j=1, nSize
             do i=1, nSize
                Amatrix(i,j) = dble( slength(ipts(i)) )**dble( nSize-j )
             enddo
          enddo
          do i=1, nSize
             xpos(i) = trajectory( x_, ipts(i) )
             ypos(i) = trajectory( y_, ipts(i) )
          enddo
          call gaussElimin( Amatrix, xcoef, xpos, nSize )
          call gaussElimin( Amatrix, ycoef, ypos, nSize )
       endif
  ...
  ! ========================================================== !
  subroutine gaussElimin( Amat, xvec, bvec, nS )
    implicit none
    integer         , intent(in)  :: nS
    double precision, intent(in)  :: Amat(nS,nS)
    double precision, intent(in)  :: bvec(nS)
    double precision, intent(out) :: xvec(nS)
    ...
    do k=1, nS
       ...
       if ( k.eq.nS ) then
          ...
          ! -- [Not Last Row :: k != nS ] -- !
          !  - Division    -  !
          Umat(k,k+1:nS) = Dinv * Umat(k,k+1:nS)
          xvec(k)           = Dinv * xvec(k)
          !  - subtraction -  !
          do j=k+1,nS
             Umat(j,k+1:nS) = Umat(j,k+1:nS) - Umat(j,k) * Umat(k,k+1:nS)
