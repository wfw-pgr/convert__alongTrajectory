program main
  ...
  character(cLen) , parameter   :: crdFile = "dat/coord_xy.dat"
  character(cLen) , parameter   :: outFile = "dat/coord_sn.dat"
  integer                       :: ik, ntrajectory, ncmp, ncoord
  character(cLen)               :: cmt
  double precision, allocatable :: trajectory(:,:), coord_xy(:,:), coord_sn(:,:)
  ...
  ! --- [2] load grid data                             --- !
  ! ------------------------------------------------------ !
  open(lun,file=trim(crdFile),status="old")
  read(lun,*)
  read(lun,*) cmt, ncoord, ncmp
  read(lun,*) cmt, ncoord, ncmp
  allocate( coord_xy(ncmp,ncoord) )
  do ik=1, ncoord
     read(lun,*) coord_xy(:,ik)
  enddo
  close(lun)

  ! ------------------------------------------------------ !
  ! --- [3] call converter routines                    --- !
  ! ------------------------------------------------------ !
  allocate( coord_sn(3,ncoord) )
  coord_sn(:,:) = 0.d0
  call convert__trajectoryCoordinate( trajectory, coord_xy, coord_sn, ntrajectory, ncoord )

  ! ------------------------------------------------------ !
  ! --- [4] save converted coordinate                  --- !
  ! ------------------------------------------------------ !
  open(lun,file=trim(outFile),status="replace")
  write(lun,"(a)") "# x_ y_ z_ i_ s_ f_"
  write(lun,"(a,2(i10,1x))") "# ", ncoord, ncmp
  write(lun,"(a,2(i10,1x))") "# ", ncoord, ncmp
  do ik=1, ncoord
     write(lun,"(3(e15.8,1x))") coord_sn(:,ik)
  enddo
  close(lun)
